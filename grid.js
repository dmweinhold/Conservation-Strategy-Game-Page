...
function handleCellClaim(scene, cell) {
  ...
  scene.tweens.add({
    ...
    onComplete: () => {
      ...
      // Mark ownership in the cell
      cell.cellData.owner = claimingTeam;
      // Optional: recolor the cell’s text to white now that it's claimed
      ...
      // If next player has no claims left, skip to the other or end game
      skipIfNoClaims(scene);

      // If next player is AI, do the AI move, else check end-game
