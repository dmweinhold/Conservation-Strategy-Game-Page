...
class MyScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Save computed dimensions for later (used by grid animations)
    ...
    if (isDesktop) {
      ...
      // For devices, place turn text below the grid.
    ...
    // Create the grid. Desktop always uses cellSize = 100.
    const gridConfig = {
      gridSize,
 ...
 * If on a device (window.innerWidth < 1024), destroy the Phaser game and replace the page with an opaque HTML results screen.
...
export function displayFinalResults(scene) {
  if (window.innerWidth < 1024) {
    ...
    container.style.background = '#6EA06E';  // opaque background
    ...
    // Play Again: set autoStart flag then reload so the game starts with the same options.
    ...
    // End & Exit: clear stored options so landing page appears.
    exitBtn.onclick = () => {
