 ...
 * 
 * - For desktop (screenWidth >= 1024): use fixed cell size (e.g. 100) 
...
class MyScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.cumGreenBAU           = 0; // how many BAU cells green has claimed
    this.cumFarmerDeduction    = 0; // how many deduc. we've applied so far
    ...
    if (isDesktop) {
      ...
      const claimsStyleF     = { font: '20px Arial', fill: '#654321' };
      const claimsStyleG     = { font: '20px Arial', fill: '#228B22' };
      ...
      this.farmerClaimsText= this.add.text(gameWidth - 220, 90, `Farmer Claims: ${this.availFarmerClaims}`, claimsStyleF);
      this.greenClaimsText = this.add.text(20, 90, `Green Claims: ${this.availGreenClaims}`, claimsStyleG);
      this.turnText        = this.add.text(gameWidth / 2, 30, `Current Turn: ${this.currentPlayer}`, 
                               { font: '24px Arial', fill: '#ffffff' }).setOrigin(0.5, 0);
    } else {
      // Mobile scoreboard
...
/**
 * displayFinalResults (the **original** approach): 
 *  - Destroys the Phaser game
 *  - Replaces entire body with final stats
 *  - "Play Again" => reload
 *  - "End & Exit" => about:blank
 */
export function displayFinalResults(scene) {
  ...
  // Destroy the Phaser instance first
...
/**
 * Start the Phaser game in "MyScene". 
 */
export function startPhaserGame(userOptions) {
  const gridSize = parseInt(userOptions.gridSize, 10) || 4;
  const dims     = computeGameDimensions(gridSize);
